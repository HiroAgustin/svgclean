  ...
  , group: function (tag)
    {
      ...
      while (children.length)
      {
        ...
        if (group)
        {
          if (this.areRelated(classes, this.getClassArray(group)))
            group.children.push(this.removeClass(child, classes[0]));

          else
          {
            if (group.name === 'g' && group.children.length === 1)
              grouped[index] = group.children[0];

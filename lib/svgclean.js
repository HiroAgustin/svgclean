  ...
  var SvgCleaner = function SvgCleaner (src, dst)
      {
        ...
        this.readStream = fs.createReadStream(path.join(process.cwd(), this.src), {
          encoding: 'utf8'
        });
    ...
    , methods = {
        ...
        init: function ()
        {
          this
            .listen()
            .readStream.pipe(this.saxStream);
      ...
      , createWriteStream: function createDest ()
        {
          mkdirp.sync(path.join(process.cwd(), path.dirname(this.dst)));
      ...
      , engraveStyles: function engraveStyles (colors)
        {
          ...
          if (keys.length)
          {
            ...
            for (index in keys)
              this.nextLine().indent().engrave('.' + keys[index] + ' {fill:' + colors[keys[index]] + ';}');
      ...
      , endHandler: function endHandler ()
        {
          ...
          this.createWriteStream()
            .on('finish', this.emit.bind(this, 'finish'));
